...
import com.ar.echoafcavlapplication.Models.DriverCardStatus;
import com.ar.echoafcavlapplication.Models.Fare;
import com.ar.echoafcavlapplication.Models.FareClass;
import com.ar.echoafcavlapplication.Models.FindCardResult;
...
import com.ar.echoafcavlapplication.SerialPort.ComBean;
import com.ar.echoafcavlapplication.SerialPort.SerialHelper;
...
public class ReaderHandler extends Service implements Runnable {
    ...
    private SerialPortWrapper samSerialPortWrapper;
    public static boolean shouldRead = true;
    private static ReaderHandler instance;
    private Integer transactionCountForUI = 0;
    private Long currentCardSerial = 0L;

    Callbacks activity;
    MainActivity mainActivity;
    private final IBinder mBinder = new ReaderHandler.LocalBinder();

    //commands
    /*private byte[] F_Card = new byte[]{0x02, 0x00, 0x04, 0x4C, 0x03, 0x00, 0x00, 0x4B};*/
    ...
    private byte[] RESET = new byte[]{ 0x02, 0x00, 0x01, 0x12, 0x00 };
    private byte[] REQA = new byte[]{ 0x02, 0x00, 0x01, 0x21, 0x00 };
    private byte[] RF_ON = new byte[]{ 0x02, 0x00, 0x01, 0x10, 0x00 };
    private byte[] RF_OFF = new byte[]{ 0x02, 0x00, 0x01, 0x11, 0x00 };
    private byte[] ANTICOLL_SELECT = new byte[]{ 0x02, 0x00, 0x01, 0x3D, 0x00 };
    ...
    private byte[] Authentication_Key_CMD = new byte[]{ 0x02, 0x00, 0x09, 0x30, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    private byte[] ReadBlock_CMD = new byte[]{ 0x02, 0x00, 0x02, 0x27, 0x00, 0x00 };
    private byte[] WriteBlock_CMD = new byte[]{ 0x02, 0x00, 0x12, 0x28, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00 };

    private byte[] MIFARE_INCREMENT_TRANSFER = new byte[]{ 0x02, 0x00, 0x07, 0x2B,
            0x00, //Source BlockNo
            0x00, 0x00, 0x00, 0x00, // Value LSBfirst
            0x00, //Target BlockNo
            0x00 //BCC
    };
    private byte[] MIFARE_DECREMENT_TRANSFER = new byte[]{ 0x02, 0x00, 0x07, 0x2C,
            0x00, //BlockNo
            0x00, 0x00, 0x00, 0x00, // Value LSBfirst
            0x00, //Target BlockNo
            0x00 //BCC
    };
    private byte[] MIFARE_INCREMENT = new byte[]{ 0x02, 0x00, 0x06, 0x29,
            0x00, //Source BlockNo
            0x00, 0x00, 0x00, 0x00, // Value LSBfirst
            0x00 //BCC
    };

    private byte[] MIFARE_DECREMENT = new byte[]{ 0x02, 0x00, 0x06, 0x2A,
            0x00, //Source BlockNo
            0x00, 0x00, 0x00, 0x00, // Value LSBfirst
            0x00 //BCC
    };

    private byte[] MIFARE_TRANSFER = new byte[]{ 0x02, 0x00, 0x02, 0x2E,
            0x00, //Source BlockNo
            0x00  //BCC
    };

    private byte[] keyA = new byte[] { (byte)0x89, 0x51, 0x71, (byte)0xf2, 0x1d, (byte)0xcb };
    private byte[] keyB = new byte[]{(byte)0x8d, 0x5f, 0x30, 0x7c, (byte)0xf0, 0x5e };
    //end-commands

    private Boolean samInitialized = false;
    private Integer samSlot = -1;
    private List<Byte> samID = null;
    private List<Byte> key = new ArrayList<>();
    private List<Byte> mainBlockData = new ArrayList<>();
    public static Boolean cardDetected = false;
    private Integer driverCardCode = 5;
    private Integer operatorCardCode = 1;
    private Integer lineAdminCardCode = 10;
    private Long staffID = 0L;

    private Date lastInterActionDate = Calendar.getInstance().getTime();
    private long IDLE_TIMEOUT = 20000;

    @Override
    public void onCreate(){
        log.error("ReaderHandler > 1");
        if (instance == null)
            instance = this;
    ...
    private void initialReader(){
        try{
            ...
            readerSerialPortWrapper.setPort(Constants.cardReaderPort);
            readerSerialPortWrapper.setBaudRate(Constants.cardReaderBaudRate);
            readerSerialPortWrapper.open();
        }catch (Exception ex){
            log.error("ReaderHandler > initialReader():" + ex.getMessage());
    ...
    private void initialReaderGPIO(){
        try{
            /*Gpio gpio = new Gpio();
            ...
            log.error("ReaderHandler > initialReaderGPIO():" + ex.getMessage());
            StringWriter errors = new StringWriter();
            ex.printStackTrace(new PrintWriter(errors));
            log.error(errors.toString());
        }
    ...
    private void initialSam(){
        try{
            samSerialPortWrapper = new SerialPortWrapper();
            samSerialPortWrapper.setPort(Constants.samPort);
            samSerialPortWrapper.setBaudRate(Constants.defSamBaudRate);
            samSerialPortWrapper.open();
        }catch (Exception ex){
            log.error("ReaderHandler > initialSam():" + ex.getMessage());
            StringWriter errors = new StringWriter();
            ex.printStackTrace(new PrintWriter(errors));
            log.error(errors.toString());
        }
    ...
    private Boolean handleSam(){
        try{
            ...
            if (findSlot()){
                //detect sam
                log.error("MO > samSlot:" + samSlot);
                if (samSelectApp()){
                    //selected app
                    if (samVerifyPin()){
                        //sam successfully initialized.
                        return true;
                    }else{
                        OutOfServiceHandler.addErrorToMap(Constants.OUT_OF_SERVICE_SAM_VERIFY, 1);
                    }
                }else{
                    OutOfServiceHandler.addErrorToMap(Constants.OUT_OF_SERVICE_SAM_SELECT, 1);
                }
            }else{
                OutOfServiceHandler.addErrorToMap(Constants.OUT_OF_SERVICE_NO_SAM, 1);
            }

        }catch (Exception ex){
            log.error("ReaderHandler > handleSam():" + ex.getMessage());
            StringWriter errors = new StringWriter();
            ex.printStackTrace(new PrintWriter(errors));
            log.error(errors.toString());
        }
    ...
    public void onDestroy() {
        try {
            ...
            if (readerSerialPortWrapper.isOpen())
                readerSerialPortWrapper.close();
            log.error("ReaderHandler service stopped...");
        }catch (Exception ex){
            log.error("ReaderHandler > onDestroy():" + ex.getMessage());
            StringWriter errors = new StringWriter();
            ex.printStackTrace(new PrintWriter(errors));
            log.error(errors.toString());
        }
    ...
    public void run() {
        while (!stopConditions) {
            try {
                /*log.error("reader while start");
                log.error("is OUS: "+ OutOfServiceHandler.isOutOfService+" , "+OutOfServiceHandler.getAllErrors());*/
                if (readerSerialPortWrapper.isOpen() && samInitialized && shouldRead){
                    //log.error("go handle card");
                    handleCard();
                }else if (readerSerialPortWrapper.isOpen() && samInitialized && !shouldRead){
                    //log.error("go handle crew card");
                    handleCrewCard();
                }
                if (activity != null) {
                    if (needGoToIDLE()) {
                        if (activity.getBrightness().equals(BrightnessType.FULL))
                            activity.setBrightness(BrightnessType.MIN);
                    } else {
                        if (activity.getBrightness().equals(BrightnessType.MIN))
                            activity.setBrightness(BrightnessType.FULL);
                    }
                }
                Thread.sleep(30);
            } catch (Exception ex) {
                log.error("ReaderHandler > run():" + ex.getMessage());
                StringWriter errors = new StringWriter();
                ex.printStackTrace(new PrintWriter(errors));
                log.error(errors.toString());
            }
    ...
    private boolean needGoToIDLE() {
        try{
            if (Math.abs(Calendar.getInstance().getTimeInMillis() - lastInterActionDate.getTime()) > IDLE_TIMEOUT) {
                if (!activity.getCurrentUiPage().equals(UiCommandTypeEnum.SUMMARY_REPORT) &&
                        !activity.getCurrentUiPage().equals(UiCommandTypeEnum.LINE_ADMIN_PAGE) &&
                        ...
                        !activity.getCurrentUiPage().equals(UiCommandTypeEnum.DRIVER_PAGE) &&
                        !activity.getCurrentUiPage().equals(UiCommandTypeEnum.UPLOAD_DOWNLOAD_PAGE))
                    return true;
            }
        }catch (Exception ex){
            log.error("ReaderHandler > needGoToIDLE():" + ex.getMessage());
            StringWriter errors = new StringWriter();
            ex.printStackTrace(new PrintWriter(errors));
            log.error(errors.toString());
        }
    ...
    public interface Callbacks {
        void showOnUi(ProcessCardResult processCardResult);
